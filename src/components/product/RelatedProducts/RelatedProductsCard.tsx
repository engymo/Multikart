...
interface RelatedProductCardProps {
    product: RelatedProduct | any;
}

export const RelatedProductCard: React.FC<RelatedProductCardProps> = ({ product }) => {
    const [isInteracted, setIsInteracted] = useState(false);

    const rawPrice = product?.price ?? product?.new_price ?? product?.final_price ?? 0;
    const priceNum = typeof rawPrice === 'number' ? rawPrice : Number(rawPrice) || 0;

    const rawOriginalPrice = product?.originalPrice ?? product?.old_price;
    const originalPriceNum = rawOriginalPrice ? Number(rawOriginalPrice) : null;

    const imageSrc = product?.image || product?.main_image || '/placeholder.png';
        ...
        <div className="group relative bg-white border border-gray-100
         rounded-sm overflow-hidden mb-10">
            ...
            <div className="relative aspect-[3/4] w-full overflow-hidden ">
                ...
                <div className="absolute bottom-3 left-3 bg-white
                 px-2 py-1 flex items-center gap-1 text-xs text-gray-700">
                    ...
                    <span className="font-semibold text-[12px]">{product?.rating ?? 5}</span>
                </div>

                {/* Hover Side Menu Actions */}
                <div className="absolute top-3 right-3 flex flex-col gap-2 z-10">
                    {RelatedIcon && RelatedIcon.map((action, index) => (
                        <button
                            key={action.icon}
            ...
            <div className="p-4 space-y-1">
                <h3 className="font-semibold text-[rgb(34,34,34)] text-base truncate">{product?.title || product?.name}</h3>
                <p className="text-xs text-gray-400 truncate">{product?.subtitle}</p>

                {/* Prices */}
                <div className="flex items-center gap-2 pt-1">
                    <span className="font-meduim text-[rgb(34,34,34)] text-[15px]">${priceNum.toFixed(2)}</span>
            ...
            <div
                ...
                <div className="relative overflow-hidden border-t border-[rgb(238,238,238)] py-1.5 px-1 select-none">
                    <div className="flex w-full overflow-hidden">
                        <div
                            className={`flex whitespace-nowrap gap-6 transition-all duration-300 ${isInteracted ? 'animate-marquee' : 'lg:group-hover:animate-marquee'
                                }`}
                        >
                            {[1, 2].map((i) => (
                                <div key={i} className="flex items-center gap-1.5 text-[14px] font-medium text-[#222222]">
                                    ...
                                    <span>{product?.offerTag || product?.offer_tag || 'Special Offer'}</span>
                                </div>
                            ))}
                        </div>
                    </div>
